...
from ..models import Region, TreePlant, TreeTypes
from ...accounts.models import Department


class AgriculturalWithoutProfitSheet(AgriculturalDBQuery):
    ...
    def generate_agricultural_without_profit_report(self):
        now = datetime.now()
        ...
        products_by_type = TreePlant.objects.filter(types__pk=self.tree_type.pk)
        profit_obj = TreeTypes.objects.get(pk=self.tree_type.pk)
        if profit_obj.name == "Бошоқли дон" or "Boshoqli don":
            sheet.merge_cells('G2:G4')
            total_ton_title = sheet.cell(column=7, row=2, value='Давлатга топширилган буғдой тонна')
            total_ton_title.alignment = Alignment(textRotation=90, wrapText=True, horizontal='center', vertical='center')
            total_ton_title.font = Font(name='Times New Roman', italic=True, bold=True, size=12)
            end_column = products_by_type.count() * 3 + 7
            col_index = 8
            begin_column = get_column_letter(col_index)  # H
        else:
            end_column = products_by_type.count() * 3 + 6
            col_index = 7
            begin_column = get_column_letter(col_index)  # G
        end_column_letter = get_column_letter(end_column)
        sheet.merge_cells(f'A1:{end_column_letter}1')
        # header_title_coordinate = f'{begin_column}2:{get_column_letter(end_column)}2'
        header_title_coordinate = f'{begin_column}2:{end_column_letter}2'
        sheet.merge_cells(header_title_coordinate)
        total_ton_title = sheet.cell(column=col_index, row=2, value='Шу жумладан')
        total_ton_title.alignment = align
        total_ton_title.font = Font(name='Times New Roman', italic=True, bold=True, size=13)

        for index, product in enumerate(products_by_type):
            ...
            units_1 = sheet.cell(row=4, column=col_index, value='га')
            units_1.font = Font(name='Times New Roman', bold=True, italic=True, size=12)
        ...
        for index, department_value in enumerate(departments):
            ...
            height_col = 4
            if profit_obj.name == "Бошоқли дон":
                start_col = 8
            else:
                start_col = 7
            ...
            for village_index, village_value in enumerate(agricultural_product):
                task_hectare = sheet.cell(column=start_col, row=row_index, value=village_value[2])
                if row_index not in task_total_each_hectare_row:
                    task_total_each_hectare_row.append(row_index)
                task_hectare.alignment = align
                task_ton = sheet.cell(column=start_col + 2, row=row_index, value=village_value[3])
                ...
                task_centner = sheet.cell(
                    column=start_col + 1, row=row_index,
                    value=task_centner_formula_1)
                task_centner.alignment = align
                task_centner.number_format = '0.0'
                # **********************************************************************
                act_hectare = sheet.cell(column=start_col, row=row_index + 1, value=village_value[4])
                if row_index + 1 not in task_total_each_ton_row:
                    task_total_each_ton_row.append(row_index + 1)
                act_hectare.alignment = align
                act_ton = sheet.cell(column=start_col + 2, row=row_index + 1, value=village_value[5])
                ...
                act_centner = sheet.cell(
                    column=start_col + 1, row=row_index + 1,
                    value=task_centner_formula_2)
            ...
            if index == departments.count() - 1 or current_region.pk != departments[index + 1].region.pk:
                ...
                for _, _ in enumerate(agricultural_product):
                    ...
                    # -----------------------------------------------------------------------------------
                    percent_region_hectare_col_formula = f'=IF({get_column_letter(column)}{row_index}=' \
                                                         f'{0},{0},{get_column_letter(column)}' \
                                                         f'{row_index}/{get_column_letter(column)}{row_index + 1}*{100})'
                    percent_region_hectare = sheet.cell(
                        column=column, row=row_index + 2, value=percent_region_hectare_col_formula)
                    ...
                    percent_region_centner_col_formula = f'=IF({get_column_letter(column + 1)}{row_index}=' \
                                                         f'{0},{0},{get_column_letter(column + 1)}{row_index}/' \
                                                         f'{get_column_letter(column + 1)}{row_index + 1}*{100})'
                    percent_region_centner = sheet.cell(
                        column=column + 1, row=row_index + 2, value=percent_region_centner_col_formula)
                    ...
                    percent_region_centner = sheet.cell(
                        column=column + 2, row=row_index + 2, value=percent_region_ton_col_formula)
                    percent_region_centner.alignment = align
                    percent_region_centner.font = Font(name='Times New Roman', bold=True, size=12)
                    percent_region_centner.number_format = '0.0'
                ...
                percent_region_hectare = sheet.cell(
                    column=height_col, row=row_index + 2, value=percent_region_hectare_formula)
                percent_region_hectare.alignment = align
                percent_region_hectare.font = Font(name='Times New Roman', bold=True, size=12)
                percent_region_hectare.number_format = '0.0'
