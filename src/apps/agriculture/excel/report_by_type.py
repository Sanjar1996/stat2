...
from ...trees.models import TreePlant


class AgriculturalByTypeSheet(AgriculturalDBQuery):
    ...
    def generate_agricultural_excel_by_type_report(self):
        now = datetime.now()
        ...
        write_header_title(sheet=sheet, row=2, col=4, size=12, is_bold=True, is_italic=True, value=self.tree_type.name)
        ...
        start_col = 0
        if str(self.tree_type.name) == "Бошоқли дон" or str(self.tree_type.name) == "Boshoqli don":
            print("1...IF", str(self.tree_type.name))
            write_header_title(sheet=sheet, row=2, col=8, size=18, is_bold=True, is_italic=True, value='Шу жумладан')
            sheet.merge_cells('G2:G4')
            total_ton_title = sheet.cell(row=2, column=7, value='Давлатга топширилган буғдой тонна')
            ...
            col_index = 8
            begin_column = get_column_letter(col_index)  # H (8)
        else:
            print("2...ELSE", str(self.tree_type.name))
            write_header_title(sheet=sheet, row=2, col=7, size=18, is_bold=True, is_italic=True, value='Шу жумладан')
            end_column = products_by_type.count() * 3 + 6
            col_index = 7
            begin_column = get_column_letter(col_index)  # G (7)
        start_col = col_index
        end_column_letter = get_column_letter(end_column)
        sheet.merge_cells(f'A1:{end_column_letter}1')
        sheet.merge_cells(f'{begin_column}2:{end_column_letter}2')
        total_ton_title = sheet.cell(row=2, column=col_index, value='Шу жумладан')
        ...
        data = self.get_data_by_tree_type(user=self.user, cat_id=self.tree_type.id, qs=products_by_type, start=self.start_date, end=self.end_date)
        ...
        for index, value in enumerate(data):
            if region_id is None or value['region_id'] != region_id:
                # print("REG ID...", value['region_id'])
                article_id = 0
                region_row.append(row_index)
                region_id = self.write_region_data(
                    sheet=sheet, row=row_index, col=start_col, region_id=value['region_id'],
                    count=value['department_count'], data=value['plan'])
                row_index += 3
            article_id += 1
            # print("DEP....", value['department'])
            ...
            self.write_plan_act_value(sheet=sheet, row=row_index, col=start_col, data=value['plan'])
            self.write_plan_act_value(sheet=sheet, row=row_index + 1, col=start_col, data=value['act'])
            self.calc_percent_value(sheet=sheet, row=row_index + 2, col=start_col, length=len(value['act']))
            self.write_d_e_f(
        ...
        for x in range(length):
            col_1 = get_column_letter(start_col)
    ...
    def write_region_data(self, sheet, row, col, region_id, count, data):
