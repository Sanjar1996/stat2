...
class AnnualReportSproutResidueSheet:
    ...
    def generate_annual_sprout_excel_report(self):
        now = datetime.now()
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = f"attachment; filename=nixol_kirim_chiqim-{now.strftime('%d-%m-%Y')}.xlsx"
        wb = Workbook()
        sheet = wb.active
        ...
        title.value = f'{self.department.name} да НИҲОЛЛАР харакати тўғрисида {self.start_date}' \
        ...
        tree_plant_list = self.prepare_sprout_residue_sql()
        category_id = 0
        category_row = []
        article_id = 0
        ...
        for index, data in enumerate(tree_plant_list):
            ...
            if index == len(tree_plant_list) - 1 or category_id != tree_plant_list[index + 1][1]:
                ...
                if qs_category.exists():
                    ...
                    row_index += 1
        write_republic_data(
            sheet=sheet, row=row_index, rep_title='Жами нихоллар',
    ...
    def prepare_sprout_residue_sql(self):
        with connection.cursor() as cursor:
            query = f"""
            SELECT tp.id, tp.category_id, tp.name, (SELECT SUM (si.donation) + SUM (si.buying) + 
            SUM (si.new_sprout) - SUM (so.for_the_forest) + SUM (so.unsuccessful) + SUM (so.place_changed) +
             SUM (so.selling) + SUM (so.donation) 
             FROM tree_plant tp1 
             LEFT JOIN sprout_input si ON si.plant_id = tp1.id AND si.date < '{self.start_date}' 
             AND si.department_id = {self.department.pk} 
             LEFT JOIN sprout_output so ON so.plant_id = tp1.id AND so.date < '{self.start_date}' 
             AND so.department_id = {self.department.pk} 
             WHERE tp1.id = tp.id) qoldiq, 
             SUM (si.donation) donation, 
             SUM (si.buying) buying, 
             SUM (si.new_sprout) new_sprout, 
             SUM (so.for_the_forest), 
             SUM (so.unsuccessful), 
             SUM (so.place_changed), 
             SUM (so.selling), 
             SUM (so.donation), 
             SUM (so.out_of_count) 
             FROM tree_plant tp 
             LEFT JOIN sprout_input si ON si.plant_id = tp.id AND si.date BETWEEN '{self.start_date}' AND '{self.end_date}' 
             AND si.department_id = {self.department.pk} 
             LEFT JOIN sprout_output so ON so.plant_id = tp.id AND so.date BETWEEN '{self.start_date}' AND '{self.end_date}' 
             AND so.department_id = {self.department.pk} 
